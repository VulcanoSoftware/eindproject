...
Deze tool draait alleen op Linux. Vanuit Windows gebruik je hem het makkelijkst via:
- WSL2 (aanrader)
...
```
Herstart als dat gevraagd wordt, en open daarna de Ubuntu app.

### Stap 2 — Projectmap in WSL zetten
Kopieer je projectmap in Windows Verkenner naar:
`\\wsl$\Ubuntu\home\<jouw-linux-user>\MultiDisk-FileBalancer\`

### Stap 3 — Eén keer dependencies installeren (in Ubuntu)
Open Ubuntu en voer uit:
```bash
...
### Stap 4 — Tool starten (eerste run maakt config.yml)
```bash
cd ~/MultiDisk-FileBalancer
python3 multidisk_filebalancer_nieuw_unstable.py
```
Beantwoord de vragen:
- WebDAV: kies `yes`, host `0.0.0.0`, port `8080`
- SFTP: kies `yes`, host `0.0.0.0`, port `8081`

### Stap 5 — Uploaden vanuit Windows
#### WebDAV (aanrader voor Verkenner)
1. Verkenner → “Deze pc”
2. “Netwerklocatie toevoegen”
3. Adres: `http://localhost:8080/`
4. Inloggen met `username/password` uit `config.yml`

#### SFTP (aanrader voor drag-and-drop)
Gebruik WinSCP:
- File protocol: SFTP
- Host name: `localhost`
- Port: `8081`
...
## Optie B — VirtualBox / VMware (Linux VM)
